...
use crate::{AnvilState, shell::FullscreenSurface};
...
impl<Backend> AnvilState<Backend> {
        ...
        if !self.pointer.is_grabbed() {
            ...
            if let Some(output) = space.output_under(self.pointer_location) {
                ...
                if let Some(window) = output.user_data().get::<FullscreenSurface>().and_then(|f| f.get()) {
                    let surface = window.surface_under(self.pointer_location - output_geo.loc.to_f64()).map(|(s, _)| s);
    ...
    fn surface_under(&self) -> Option<(WlSurface, Point<i32, Logical>)> {
        ...
        } else if let Some(layer) = layers.layer_under(WlrLayer::Overlay, pos).or_else(|| layers.layer_under(WlrLayer::Top, pos)) {
...
impl AnvilState<UdevData> {
        ...
        match event {
            InputEvent::Keyboard { event, .. } => match self.keyboard_key_to_action::<B>(event) {
                ...
                KeyAction::Screen(num) => {
                    let space = self.space.borrow();
                    let geometry = space.outputs().skip(num).next().map(|o| space.output_geometry(o).unwrap());
                ...
                KeyAction::ScaleUp => {
                    ...
                    if let Some(output) = output {
                        let (output_location, scale) = (space.output_geometry(&output).unwrap().loc, space.output_scale(&output).unwrap());
    ...
    fn clamp_coords(&self, pos: Point<f64, Logical>) -> Point<f64, Logical> {
        ...
        let max_x = space.outputs().fold(0, |acc, o| acc + space.output_geometry(o).unwrap().size.w);
        let clamped_x = pos_x.max(0.0).min(max_x as f64);
        let max_y = space.outputs().find(|o| {
