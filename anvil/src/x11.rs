...
use smithay::{
    backend::{
        ...
        renderer::{gles2::Gles2Renderer, Bind, ImportEgl, Renderer, Frame, Unbind},
...
use crate::{
    drawing::*,
    state::Backend,
    AnvilState, shell::FullscreenSurface,
};
        ...
        if state.backend_data.render {
            ...
            let render_res = if let Some(window) = output.user_data().get::<FullscreenSurface>().and_then(|f| f.get()) {
                ...
                let res = renderer.render(mode.size, transform, |renderer, frame| {
                    ...
                    draw_window(renderer, frame, &window, scale, (0, 0), &[damage.to_f64().to_logical(scale).to_i32_round()], &log)?;
                ...
                space.render_output(&mut *renderer, &output, age as usize, CLEAR_COLOR, &*elements).map(|x| x.is_some())
            };
            match render_res {
                Ok(true) => {
