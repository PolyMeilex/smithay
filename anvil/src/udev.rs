...
use smithay::{
    backend::{
        ...
        renderer::{
            gles2::{Gles2Renderer, Gles2Frame, Gles2Texture, Gles2Error},
    ...
    desktop::{space::{Space, RenderElement, RenderError}, draw_window, Window},
    reexports::{
        calloop::{
    ...
    wayland::{
        output::{Output, Mode, PhysicalProperties},
...
use crate::{
    drawing::*,
    state::{AnvilState, Backend}, shell::FullscreenSurface,
};
        ...
        'outer: for encoder_info in encoder_infos {
            for crtc in res_handles.filter_crtcs(encoder_info.possible_crtcs()) {
                if let Entry::Vacant(entry) = backends.entry(crtc) {
                    ...
                    let position = (space.outputs().fold(0, |acc, o| acc + space.output_geometry(o).unwrap().size.w), 0).into();
...
impl AnvilState<UdevData> {
        ...
        if let Some(ref mut backend_data) = self.backend_data.backends.get_mut(&device) {
            ...
            let relocate_windows = space
                ...
                .map(|o| {
                    ...
                    let windows = space.windows().filter_map(|w| {
                        let mut w_geo = space.window_geometry(w).unwrap();
                        if geo.contains(w_geo.loc) {
                            w_geo.loc -= geo.loc;
                            Some((w.clone(), (geo.size.w as f64 / w_geo.loc.x as f64, geo.size.h as f64 / w_geo.loc.y as f64)))
                        } else {
                            None
                        }
                    }).collect::<Vec<_>>();
            ...
            *backends = scan_connectors(
            ...
            for (output, window) in relocate_windows.into_iter() {
                let new_output_geo = space.outputs().find(|o| o.name() == output.name())
                ...
                for (window, (x_offset, y_offset)) in window {
                    let offset = Point::from((x_offset * new_output_geo.size.w as f64, y_offset * new_output_geo.size.h as f64)).to_i32_round();
    ...
    fn device_removed(&mut self, device: dev_t) {
        ...
        if let Some(backend_data) = self.backend_data.backends.remove(&device) {
            ...
            let i = space.outputs().enumerate().find(|(_, output)| {
                output
                    .user_data()
                    .get::<UdevOutputId>()
            ...
            for (output, windows) in relocate_windows.into_iter().skip(1) {
                let position = (space.outputs().fold(0, |acc, o| acc + space.output_geometry(o).unwrap().size.w), 0);
                space.map_output(&output, 1.0, position);

                let new_output_loc = space.outputs().find(|o| o.name() == output.name())
                    .or_else(|| space.outputs().next())
...
fn render_surface(
    ...
    let output = if let Some(output) = space.outputs()
    ...
    let mut elements: Vec<Box<dyn RenderElement<Gles2Renderer, Gles2Frame, Gles2Error, Gles2Texture> + 'static>> = Vec::new();
    // set cursor
    if output_geometry.to_f64().contains(pointer_location) {
        ...
        #[cfg(feature = "debug")]
        {
            elements.push(Box::new(draw_fps(
                fps_texture,
    ...
    let render_res = if let Some(window) = output.user_data().get::<FullscreenSurface>().and_then(|f| f.get()) {
        let transform = output.current_transform().into();
        let mode = output.current_mode().unwrap();
        let scale = space.output_scale(&output).unwrap();
        let res = renderer.render(mode.size, transform, |renderer, frame| {
            let damage = Rectangle::from_loc_and_size((0, 0), mode.size);
            frame.clear(CLEAR_COLOR, &[damage])?;
            draw_window(renderer, frame, &window, scale, (0, 0), &[damage.to_f64().to_logical(scale).to_i32_round()], &logger)?;
            ...
            Ok(())
        }).and_then(std::convert::identity).map(|_| true).map_err(RenderError::<Gles2Renderer>::Rendering);
        window.send_frame(start_time.elapsed().as_millis() as u32);
        res
    } else {
        space.render_output(renderer, &output, age as usize, CLEAR_COLOR, &*elements).map(|x| x.is_some())
