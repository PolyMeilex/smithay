...
impl Space {
    ...
    pub fn output_geometry(&self, o: &Output) -> Option<Rectangle<i32, Logical>> {
        ...
        o.current_mode().map(|mode| {
            Rectangle::from_loc_and_size(
                state.location,
                transform.transform_size(mode.size).to_f64().to_logical(state.render_scale).to_i32_round(),
            )
        })
    }
    ...
    pub fn render_output<R>(
        ...
        age: usize,
        clear_color: [f32; 4],
        custom_elements: &[Box<dyn RenderElement<
            R,
            <R as Renderer>::Frame,
            <R as Renderer>::Error,
    ...
    where
        ...
        R::Frame: 'static,
    {
        type SpaceElem<R> = dyn SpaceElement<R, <R as Renderer>::Frame, <R as Renderer>::Error, <R as Renderer>::TextureId>;
        ...
        for element in self.windows.iter()
            .map(|w| w as &SpaceElem<R>)
            .chain(layer_map.layers().map(|l| l as &SpaceElem<R>))
            .chain(custom_elements.iter().map(|c| c as &SpaceElem<R>))
        {
            ...
            if old_geo.map(|old_geo| old_geo != geo).unwrap_or(false) {
                ...
                damage.extend(element.accumulated_damage(Some((self, output))).into_iter().map(|mut rect| {
                    rect.loc += loc;
                    rect
                }));
            }
                    ...
                    .chain(custom_elements.iter().map(|c| c as &SpaceElem<R>))
                {
                    ...
                    if damage.iter().any(|d| d.overlaps(geo)) {
                        ...
                        let damage = damage.iter()
                            ...
                            .collect::<Vec<_>>();
                        slog::trace!(self.logger, "Rendering toplevel at {:?} with damage {:#?}", geo, damage);
                        element.draw(self.id, renderer, frame, state.render_scale, loc, &damage, &self.logger)?;
                    }
                };
        ...
        state.last_state = self.windows.iter()
            .map(|w| w as &SpaceElem<R>)
            .chain(layer_map.layers().map(|l| l as &SpaceElem<R>))
            .chain(custom_elements.iter().map(|c| c as &SpaceElem<R>))
            .map(|elem| {
