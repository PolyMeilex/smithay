...
use crate::{
    backend::{
        ...
        input::InputEvent,
        renderer::{
            gles2::{Gles2Error, Gles2Renderer}, Bind,
        },
    },
    utils::{Logical, Physical, Size, Rectangle},
};
    ...
    L: Into<Option<::slog::Logger>>,
{
    ...
    let damage_tracking = display.extensions.iter().any(|ext| ext == "EGL_EXT_buffer_age") &&
        display.extensions.iter().any(|ext| ext == "EGL_KHR_swap_buffers_with_damage" || ext == "EGL_EXT_swap_buffers_with_damage");
...
impl WinitGraphicsBackend {
    ...
    pub fn submit(&mut self, damage: Option<&[Rectangle<i32, Logical>]>, scale: f64) -> Result<(), crate::backend::SwapBuffersError>
    {
        let mut damage = if self.damage_tracking && damage.is_some() && !damage.unwrap().is_empty() {
            let size = self.size.borrow().physical_size.to_f64().to_logical(scale).to_i32_round::<i32>();
            let damage = damage.unwrap().iter().map(|rect| {
                Rectangle::from_loc_and_size(
                    (rect.loc.x, size.h - rect.loc.y - rect.size.h),
                    rect.size,
                ).to_f64().to_physical(scale).to_i32_round::<i32>()
            }).collect::<Vec<_>>();
            Some(damage)
        } else {
            None
