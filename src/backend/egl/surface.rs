...
use crate::utils::{Rectangle, Physical};
...
impl EGLSurface {
    ...
    pub fn buffer_age(&self) -> i32 {
        ...
        let mut age = 0;
        unsafe { ffi::egl::QuerySurface(**self.display, surface as *const _ , ffi::egl::BUFFER_AGE_EXT as i32, &mut age as *mut _); }
        age
    }

    /// Swaps buffers at the end of a frame.
    pub fn swap_buffers(&self, damage: Option<&mut [Rectangle<i32, Physical>]>) -> ::std::result::Result<(), SwapBuffersError> {
        let surface = self.surface.load(Ordering::SeqCst);
